...
//     var submitButton = document.querySelector('#_form_490_submit');
...
//                 _show_error("490", "Sorry, your submission failed. Please shorten your responses and try again.");
//             } else {
//                 _show_error("490", "Sorry, your submission failed. Please try again.");
//             }
//             submitButton.disabled = false;
//             submitButton.classList.remove('processing');
//         }
...
//     }
//     var _removed = false;
//         var form_to_submit = document.getElementById('_form_490_');
...
//             var submitButton = e.target.querySelector('#_form_490_submit');
//             submitButton.disabled = true;
//             submitButton.classList.add('processing');
//                         var serialized = _form_serialize(
//                 document.getElementById('_form_490_')
//             ).replace(/%0A/g, '\\n');
...
new Glide('.glide-campaign1', configCampaignGlide).mount();
new Glide('.glide-campaign2', configCampaignGlide2).mount();
new Glide('.glide-campaign3', configCampaignGlide2).mount();
new Glide('.glide-campaign4', configCampaignGlide2).mount();
...
window._load_script = function(url, callback, isSubmit) {
  ...
  script.onerror = function() {
    if (isSubmit) {
      if (script.src.length > 10000) {
        _show_error("424", "Sorry, your submission failed. Please shorten your responses and try again.");
      } else {
        _show_error("424", "Sorry, your submission failed. Please try again.");
      }
    }
  }
...
(function() {
  ...
  var _removed = false;
  var form_to_submit = document.getElementById('_form_424_');
  var allInputs = form_to_submit.querySelectorAll('input, select, textarea'), tooltips = [], submitted = false;
  ...
  var form_submit = function(e) {
    ...
    if (validate_form()) {
      // use this trick to get the submit button & disable it using plain javascript
      document.querySelector('#_form_424_submit').disabled = true;
            var serialized = _form_serialize(document.getElementById('_form_424_')).replace(/%0A/g, '\\n');
      var err = form_to_submit.querySelector('._form_error');
      err ? err.parentNode.removeChild(err) : false;
      _load_script('https://akoskarova.activehosted.com/proc.php?' + serialized + '&jsonp=true', null, true);
    }
...
new Glide('.glide-students-feedback', configExperience).mount();
 ...
 (function() {
   ...
   var _removed = false;
   var form_to_submit = document.getElementById('_form_203_');
   var allInputs = form_to_submit.querySelectorAll('input, select, textarea'), tooltips = [], submitted = false;
   ...
   var form_submit = function(e) {
     ...
     if (validate_form()) {
       // use this trick to get the submit button & disable it using plain javascript
       document.querySelector('#_form_203_submit').disabled = true;
             var serialized = _form_serialize(document.getElementById('_form_203_')).replace(/%0A/g, '\\n');
       var err = form_to_submit.querySelector('._form_error');
       err ? err.parentNode.removeChild(err) : false;
