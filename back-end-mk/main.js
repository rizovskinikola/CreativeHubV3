...
//         _show_error("420", "Sorry, your submission failed. Please shorten your responses and try again.");
//       } else {
//         _show_error("420", "Sorry, your submission failed. Please try again.");
//       }
//     }
//   }
...
//   }
//   var _removed = false;
//   var form_to_submit = document.getElementById('_form_420_');
...
//       document.querySelector('#_form_420_submit').disabled = true;
//             var serialized = _form_serialize(document.getElementById('_form_420_')).replace(/%0A/g, '\\n');
...
var typeString = " <strong>B</strong>,<strong>a</strong>,<strong>c</strong>,<strong>k</strong>,<strong>-</strong>,<strong>E</strong>,<strong>n</strong>,<strong>d</strong>, ,<span>><span>, ,А,к,а,д,е,м,и,ј,а"

var chars = typeString.split(',');
var headerTitle = document.getElementById("headerTitle");

var i = 0;
setInterval(function () {
    if (i < chars.length) {
        headerTitle.innerHTML += chars[i++];
    } else {
        i = 0;
        headerTitle.innerHTML = "<span><<span>";
    }
}, 180);
...
window._load_script = function(url, callback, isSubmit) {
  ...
  script.onerror = function() {
    if (isSubmit) {
      if (script.src.length > 10000) {
        _show_error("416", "Sorry, your submission failed. Please shorten your responses and try again.");
      } else {
        _show_error("416", "Sorry, your submission failed. Please try again.");
      }
    }
  }
...
(function() {
  ...
  var _removed = false;
  var form_to_submit = document.getElementById('_form_416_');
  var allInputs = form_to_submit.querySelectorAll('input, select, textarea'), tooltips = [], submitted = false;
  ...
  var form_submit = function(e) {
    ...
    if (validate_form()) {
      // use this trick to get the submit button & disable it using plain javascript
      document.querySelector('#_form_416_submit').disabled = true;
            var serialized = _form_serialize(document.getElementById('_form_416_')).replace(/%0A/g, '\\n');
      var err = form_to_submit.querySelector('._form_error');
      err ? err.parentNode.removeChild(err) : false;
      _load_script('https://akoskarova.activehosted.com/proc.php?' + serialized + '&jsonp=true', null, true);
    }
...
window._load_script = function(url, callback, isSubmit) {
  ...
  script.onerror = function() {
    if (isSubmit) {
      if (script.src.length > 10000) {
        _show_error("398", "Sorry, your submission failed. Please shorten your responses and try again.");
      } else {
        _show_error("398", "Sorry, your submission failed. Please try again.");
      }
    }
  }
...
(function() {
  ...
  var _removed = false;
  var form_to_submit = document.getElementById('_form_398_');
  var allInputs = form_to_submit.querySelectorAll('input, select, textarea'), tooltips = [], submitted = false;
  ...
  var form_submit = function(e) {
    ...
    if (validate_form()) {
      // use this trick to get the submit button & disable it using plain javascript
      document.querySelector('#_form_398_submit').disabled = true;
            var serialized = _form_serialize(document.getElementById('_form_398_')).replace(/%0A/g, '\\n');
      var err = form_to_submit.querySelector('._form_error');
      err ? err.parentNode.removeChild(err) : false;
      _load_script('https://akoskarova.activehosted.com/proc.php?' + serialized + '&jsonp=true', null, true);
    }
