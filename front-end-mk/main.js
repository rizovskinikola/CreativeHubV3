...
//   },
// };
// new Glide(".glide-modules", moduleMobile).mount();
...
//   },
// };
// new Glide(".glide-real-projects", configRealProjects).mount();

// student profiles percentage
...
const observer1 = new IntersectionObserver((entries) => {
  ...
  entries.forEach((entry) => {
    ...
    if (entry.isIntersecting) {
      ...
      counter("count2", 0, 19, 1100);
      counter("count4", 0, 16, 1400);
      counter("count5", 0, 41, 1500);
      counter("count6", 0, 55, 1500);
      counter("count7", 0, 45, 1500);
    }
  });
});

observer1.observe(document.querySelector(".percent-bottom"));
...
(function() {
  ...
  var form_submit = function(e) {
    ...
    if (validate_form()) {
      // use this trick to get the submit button & disable it using plain javascript
      document.querySelector('#_form_282_submit').disabled = true;
            var serialized = _form_serialize(document.getElementById('_form_282_')).replace(/%0A/g, '\\n');
      var err = form_to_submit.querySelector('._form_error');
      err ? err.parentNode.removeChild(err) : false;
      _load_script('https://akoskarova.activehosted.com/proc.php?' + serialized + '&jsonp=true', null, true);
    }
